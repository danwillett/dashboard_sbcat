...
// mui
import { FormControl, FormGroup, FormLabel, RadioGroup, Radio, InputLabel, MenuItem, Slider, Box, Typography, FormControlLabel, Checkbox } from "@mui/material";
...
export default function VolumeFilters() {
        ...
        const localityResults = await Promise.all( 
            countGroupLayer?.layers.map( async (layer) => {
                ...
                if (layer instanceof FeatureLayer) {
                    ...
                    const localityResults: FeatureSet = await layer.queryFeatures(localityQuery)
                    const uniqueLocalities = localityResults.features.map(f => f.attributes.locality)
    ...
    const filterLocalities = () => {
        ...
        if (mixedValues) {
            const includedLocalities = Object.entries(localities).filter(([_, val]) => Boolean(val))
            if (includedLocalities) {
                
                if (includedLocalities.length > 1) {
                    const quotedLocalities = includedLocalities.map(([l, _]) => `'${l}'`).join(", ");
        ...
        const programResults = await Promise.all( 
            countGroupLayer?.layers.map( async (layer) => {
                ...
                if (layer instanceof FeatureLayer) {
                    ...
                    const programResults: FeatureSet = await layer.queryFeatures(programQuery)
                    const uniquePrograms = programResults.features.map(f => f.attributes.source)
    ...
    const changeRenderField = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filterPrograms = () => {
        ...
        if (mixedValues) {
            const includedPrograms = Object.entries(programs).filter(([_, val]) => Boolean(val))
            if (includedPrograms) {
                
                if (includedPrograms.length > 1) {
                    const quotedLocalities = includedPrograms.map(([l, _]) => `'${l}'`).join(", ");
    ...
    return (
        <Box sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
            ...
            <Box sx={{width: '100%'}}>
                ...
                <FormControl fullWidth sx={{mb: 3}}>
                    <FormGroup>
                        { Object.entries(localities).map(([locale, value]) => (
                            <FormControlLabel key={locale} control={<Checkbox checked={value} onChange={handleLocaleChange} name={locale} />} label= {locale} />
                        ))}
                ...
                {/* Count Program */}
                <Typography variant='body1' align="left" my={1} sx={{width: '100%'}}>Filter by Count Program</Typography>
                <FormControl fullWidth sx={{mb: 3}}>
                    <FormGroup>
                        { Object.entries(programs).map(([program, value]) => (
                            <FormControlLabel key={program} control={<Checkbox checked={value} onChange={handleProgramChange} name={program} />} label= {program} />
                        ))}
                        {/* <FormControlLabel control={<Checkbox checked={dataSources.SWITRS} onChange={handleSourceChange} name="SWITRS" />} label="SWITRS" />
                        <FormControlLabel control={<Checkbox checked={dataSources.BikeMaps} onChange={handleSourceChange} name="BikeMaps" />} label="BikeMaps" /> */}
                    </FormGroup>
